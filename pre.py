...
def blockdiag(tensor_list, mode, p_inverse=True):
    ...
    if p_inverse == False:
        ...
        for index in range(R):
            ans_mat[index * blockdiag_row:(index + 1) * blockdiag_row,
            index * blockdiag_col:(index + 1) * blockdiag_col] = unfold(tensor_list[index], mode).T
    if p_inverse == True:
        blockdiag_row, blockdiag_col = change(blockdiag_row, blockdiag_col)
        ans_mat = np.zeros([blockdiag_row * R, blockdiag_col * R])
        for index in range(R):
            ans_mat[index * blockdiag_row:(index + 1) * blockdiag_row,
            index * blockdiag_col:(index + 1) * blockdiag_col] = pinv(unfold(tensor_list[index], mode).T)
    return ans_mat
...
def partial_tucker(tensor, modes, ranks=None, init='SVD', n_iter_max=100,
    ...
    if init == 'SVD':
        for mode in modes:
            index = mode - 1
            eugenics, _, _ = partial_svd(unfold(tensor, mode), n_eigenvecs=ranks[index])
            factors.append(eugenics)
    else:
        rng = check_random_state(random_state)
        core = np.array(rng.random_sample(ranks))
        factors = [np.array(rng.random_sample((tensor.shape[mode - 1], ranks[index]))) for (index, mode) in
                   enumerate(modes)]
    ...
    for iteration in range(n_iter_max):
        for mode in modes:
            ...
            eugenics, _, _ = partial_svd(unfold(core_approximate, mode), n_eigenvecs=ranks[index])
            factors[index] = eugenics
...
def block_term_tensor_decomposition(tensor, modes, n_part, ranks=None, n_iter_max=100, tol=10e-5, random_state=None):
    ...
    factors = [np.array(rng.random_sample((tensor.shape[index], ranks[index] * n_part))) for index, mode in
               enumerate(modes)]
    ...
    for iteration in range(n_iter_max):
        for mode in modes:
            index = mode - 1
            factors[index] = (
                blockdiag(core, mode).dot(pinv(multi_mat_kr(factors, R=n_part, mode=mode))).dot(
                    unfold(tensor, mode).T)).T
            for i in range(n_part):
                factors[index][:, i * ranks[index]:(i + 1) * ranks[index]], _ = QR(
